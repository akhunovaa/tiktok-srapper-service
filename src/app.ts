...
app.get('/trend', async (req, res) => {
    ...
    try {
        const posts = await trend('', {number: 6, noWaterMark: true, verifyFp: '', proxy: proxyList});
        console.log(posts);
        res.send(posts);
    } catch (error) {
        console.log(error);
...
app.get('/trend/:count', async (req, res) => {
    ...
    try {
        const posts = await trend('', {number: count, noWaterMark: true, verifyFp: '', proxy: proxyList});
        console.log(posts);
        res.send(posts);
    } catch (error) {
        console.log(error);
...
app.get('/user/:user/:count/:byUserId', async (req, res) => {
    ...
    try {
        const posts = await user(username, {number: count, noWaterMark: true, verifyFp: '', by_user_id: byUserId});
        console.log(posts);
        res.send(posts);
    } catch (error) {
        console.log(error);
